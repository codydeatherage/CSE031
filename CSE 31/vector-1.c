...
void vector_set(vector_t *v, size_t loc, int value) {
	...
	if(loc < v->size) {
		...
		vector_t *backupv = vector_new();
		backupv->size = v->size;
		backupv->data = (int *)malloc((v->size) * sizeof(int));
		for(int i = 0; i < v->size; ++i) {
			backupv->data[i] = v->data[i];
		}
		vector_delete(v);
		...
		for(int i = 0; i < backupv->size; ++i) {
			v->data[i] = backupv->data[i];
		}
		for(int i = backupv->size; i < loc + 1; ++i) {
			v->data[i] = 0;
		}
		v->data[loc] = value;
		vector_delete(backupv);
	}
}
